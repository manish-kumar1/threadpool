...
int main(int argc, char* argv[])
{
  ...
  auto p0 = thp::util::make_task<void>([] { std::cerr << "Hello world" << std::endl; });
  // task with priority setup
  auto p1 = thp::util::make_task<unsigned, float>(factorial, 3); p1->set_priority(-30.5f);
  auto p2 = thp::util::make_task<unsigned, float>(factorial, 4); p2->set_priority(-20.5f);

  std::vector<decltype(p1)> tasks;
  auto p3 = thp::util::make_task<unsigned, float>(factorial, 5); p3->set_priority(-30.5f);
